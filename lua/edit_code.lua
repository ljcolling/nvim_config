...
vim.api.nvim_create_user_command("EditCode", function()
  ...
  local extmarks = make_extmarks(code_block and code_block.start_row, code_block and code_block.end_row)

  vim.cmd([[new]])
  -- FIXME: create file in the same folder at the current file
  -- filename could be the current file with a uuid appended
  vim.cmd("file " .. os.tmpname())
  local bufnr = vim.api.nvim_get_current_buf()
  -- print(bufnr)
  ---@diagnostic disable-next-line: need-check-nil
  vim.bo.filetype = code_block and code_block.lang
  vim.api.nvim_buf_set_lines(0, 0, -1, true, code_block and code_block.line)
  ...
  vim.api.nvim_create_autocmd({ 'BufWritePost', 'WinClosed' }, {
    ...
    callback = function()
      ...
      if tbl_equal(lines, code_block and code_block.lines) then
        return
      end
